
public class testrun {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
		
		
		Oddeven Check_01  = new Oddeven();
		
		//Check_01.numchk();
		
		MinMaxInteger mm = new MinMaxInteger();
		
		//mm.getMinMaxInteger();
		
		MultipleTester mt = new MultipleTester();
		
		//mt.testMultiples();
		
		CheckerboardPattern cbp = new CheckerboardPattern();
		
		//cbp.displayChecker();
		
		CircleValues cv = new CircleValues();
		
		//cv.showCircleValues();
		
		CharacterValue chrvalue = new CharacterValue();
		
		//chrvalue.getCharacterValue();
		
		SeparateDigits sd = new SeparateDigits();
		
		//sd.showDigits();
		
		TableSqrCube tsc = new TableSqrCube();
		
		//tsc.getTableSqrCube();
		
		ValueofNumber vnum = new ValueofNumber();
		
		//vnum.getValueofNumber();
		
		BmiCalc bmicalc = new BmiCalc();
		//bmicalc.getBmi();
		
		Invoice displayinvoice = new Invoice();
		//displayinvoice.generateInvoice();
		
		Employee emp1 = new Employee();
		//emp1.testseq();
		
		Employee emp2 = new Employee();
		//emp2.testseq();
		
		HashMapTest hmt = new HashMapTest();
		
		hmt.testhm();
	}

}
